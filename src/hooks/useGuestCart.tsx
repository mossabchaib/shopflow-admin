...
export function useGuestCart() {
  ...
  const addItem = useCallback((product_id: string, size_id: string | null, quantity: number = 1) => {
    setItems(prev => {
      const existing = prev.find(i => i.product_id === product_id && i.size_id === size_id);
      if (existing) {
        return prev.map(i =>
      ...
      return [...prev, { product_id, size_id, quantity }];
    });
  }, []);

  const removeItem = useCallback((product_id: string, size_id: string | null) => {
    setItems(prev => prev.filter(i => !(i.product_id === product_id && i.size_id === size_id)));
  }, []);

  const updateQuantity = useCallback((product_id: string, size_id: string | null, quantity: number) => {
    if (quantity < 1) return;
    setItems(prev =>
      prev.map(i =>
        i.product_id === product_id && i.size_id === size_id ? { ...i, quantity } : i
      )
    );
  }, []);
