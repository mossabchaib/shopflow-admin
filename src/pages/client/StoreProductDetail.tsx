...
const StoreProductDetail = () => {
  ...
  useEffect(() => {
    ...
    const fetchAll = async () => {
      ...
      const [prodRes, imgRes, sizeRes, colorRes, varRes] = await Promise.all([
        supabase.from("products").select("*, categories(id, name)").eq("id", productId).single(),
        supabase.from("product_images").select("*").eq("product_id", productId).order("sort_order"),
        supabase.from("product_sizes").select("*").eq("product_id", productId).order("size_label"),
        supabase.from("product_colors").select("*").eq("product_id", productId).order("color_name"),
      ...
      if (prodRes.data?.category_id && store) {
      ...
      if (user) {
        const { data: fav } = await supabase.from("favorites").select("id").eq("user_id", user.id).eq("product_id", productId).maybeSingle();
        setIsFav(!!fav);
      }
  ...
  const addToCart = async () => {
    ...
    if (user) {
      ...
      addItem(productId!, selectedSize, quantity, selectedColor);
    }
