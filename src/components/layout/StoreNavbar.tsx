...
import { ShoppingCart, Heart, Menu, X, LogOut, LayoutDashboard, Globe, Sun, Moon } from "lucide-react";
...
export function StoreNavbar() {
  ...
  const { data: isOwner } = useQuery({
    queryKey: ["is-store-owner", user?.id, store?.id],
    queryFn: async () => {
  ...
  const links = [
    ...
    { href: `${basePath}/shop`, label: t("nav.shop") },
  ];
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          <Link to={basePath} className="flex items-center gap-2">
            ...
            <span className="font-bold text-lg text-foreground">{store?.store_name}</span>
          </Link>
          ...
          <div className="flex items-center gap-1.5">
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end">
                {languages.map(l => (
                  <DropdownMenuItem key={l.code} onClick={() => setLang(l.code)} className={lang === l.code ? "bg-primary/10 text-primary" : ""}>
                    {l.label}
                  </DropdownMenuItem>
                ))}
              </DropdownMenuContent>
